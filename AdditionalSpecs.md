...
| `release` | `semver`, `git_tag`, and one `artefact:<name>` pair per versioned artefact (`src` today; `agent_contract`/`data` once those tickets land); absent except on the entry `freeze_release()` writes for an actual release |
