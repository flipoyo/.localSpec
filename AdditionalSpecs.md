...
| `git_repo.py` | 0 | Per-repository identity types, state enumerations, provider registry, remote-URL construction. Also `RepoScope`, the one definition of which repositories a tree-wide command may touch: `private` marks a repository that configures the project rather than being it, shared with the author's other projects, read-only unless the `.cgs` entry adds `writable = true`. `RepoScope.includes` reads `effective_private`/`effective_writable`, the flags after `git_tree.propagate_privacy` has pushed each parent's privacy down; the declared `private`/`writable` are what gets serialized. `RepoScope` itself does not know about the memory mount — `merge`/`tag`/`freeze-release` need it to stay an ordinary private/writable repository, the same as `.localSpec`/`.claude`, so they go on reconciling it across project branches. `WorkingRepo.is_memory_mount` (set by `registry.py`, from `relative_path == memory.repository.MOUNT_PATH`) is read one level up instead, by `operations.iter_write_scope` — used by `add_tree`/`commit_tree`/`push_tree` (leaf-first) and `_restart_tree` — the shared body of `restart_tree`/`restart_tree_force`, behind `pull`/`pull-force` — in its own parent-first order, since all of these record their own operation into the memory *after* they run (`memory-dev_MemoryScopeExclusion`, `memory-dev_PullMemoryExclusion`, archived 2026-09-17). This stops the memory's own git state from being touched; it does not and cannot stop it from showing dirty afterward, since the record of the command that just ran is exactly what makes it so — `status` explains that rather than hiding it (`memory_dirty` on `_StatusView`). Excluding the memory from every write scope also excluded it from the refresh that keeps a repository's *recorded* commit honest, so `orchestre.write_gts_snapshot` separately re-reads the memory mount's actual HEAD — read-only, every State, regardless of which command triggered it — before building the document, or `status`'s `HEAD ending with *` marker would have appeared the first time `memory push` moved it and never cleared again (`memory-dev_MemoryRecordedRefresh`, archived same day). That same read-only refresh (`orchestre._refresh_memory_mount_state`) also marks the memory mount `READY` once it has confirmed a real branch and HEAD, for the same reason: excluding it from `_restart_tree`'s pull loop also excluded it from the *only* code that used to set `repo_lifecycle_state`, so a freshly onboarded memory could never make `WorkingGitTree.is_ready()` true again — `restart()` calls the refresh, and recomputes tree state, right before its own readiness check (`memory-dev_MergeMemoryExclusion`, archived same day). `merge`'s *action* is scoped the same way `tag`/`freeze-release` are (plain `iter_tree_leaf_first` — reconciling the memory across branches like `.localSpec`/`.claude`), and measures a private repo against its own declared branch via `git_branch.resolve_propagated_ref`; its *preflight*, unlike the action, does know about the memory mount — `_collect_worktree_diagnostics`/`_collect_tracking_diagnostics` (`operations.py`) skip `is_memory_mount` entries, since `require_clean=True` and `BEHIND`/`DIVERGED` being blocking would otherwise refuse every merge once a memory is mounted, over exactly the dirtiness and tracking-lag the two tickets above made permanent by design (`memory-dev_MergeMemoryExclusion`). |
...
| `operations.py` | 2 | Leaf/parent-first Git operations over a `WorkingGitTree` + `GitRunner`. Asks `git_tree_branch.py` which branch the tree is on and which branch each repository should follow, rather than working it out per call site — `checkout_tree`, `branch_tree`, `add_tree`, `commit_tree`, `push_tree`, `tag_tree`, `freeze_release_tree`, branch-topology validation. Requires a `READY` tree; raises `TreeNotReadyError` otherwise. `add_tree`/`commit_tree`/`push_tree` each return one `RepoOutcome` per repository they visited — what changed there, or why nothing did — so a sweep that wrote nowhere is distinguishable from one that wrote everywhere. Those three, plus `_restart_tree` behind `pull`/`pull-force`, iterate via `iter_write_scope` rather than `iter_tree_leaf_first`/`iter_tree` directly, skipping `WorkingRepo.is_memory_mount`: each records its own operation into the memory after it runs, so sweeping the memory into that same commit — or fast-forwarding, or discarding, its own worktree — could never leave it clean and, for a genuine remote divergence, could fail outright over a repository nothing else in the sweep needed. The client stores the tuple on `ComplexGitSyncClient.last_write_outcomes` and `cli/` prints it; deciding what happened stays here. `remove_paths` reports the same way, and is the one scoped operation handed its paths rather than sweeping for them: its scope is checked against the repository each path resolves to — a filter — and one path outside it refuses the whole call before any removal. Bare `rm` keeps its original reach (`RepoScope.ALL`); `cli/_shared.py` warns when that reach lands in a configuration repository. Preflight (`_run_preflight_checks` and the `_collect_*_diagnostics` functions it calls) knows about the memory mount even though the *actions* mostly do not: `_collect_worktree_diagnostics`/`_collect_tracking_diagnostics` skip `is_memory_mount` entries regardless of scope, so `merge`'s `require_clean=True` and its always-blocking `BEHIND`/`DIVERGED` never refuse a merge over the memory's own expected dirtiness or its lag behind an origin `memory push` alone is responsible for syncing (`memory-dev_MergeMemoryExclusion`, archived 2026-09-17). |
